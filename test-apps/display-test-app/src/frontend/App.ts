...
import {
  ElectronApp,
  ElectronAppOpts,
} from "../../../../core/electron/src/frontend/ElectronApp";
...
import {
  MobileApp,
  MobileAppOpts,
} from "../../../../core/mobile/src/frontend/MobileApp";
import {
...
class PushChangesTool extends Tool {
  ...
  public override async run(description?: string): Promise<boolean> {
    if (!description || "string" !== typeof description) return false;

    const imodel = IModelApp.viewManager.selectedView?.iModel;
    if (!imodel || !imodel.isBriefcaseConnection()) return false;
...
export class DtaIpc {
  public static async callBackend<T extends AsyncMethodsOf<DtaIpcInterface>>(
    methodName: T,
    ...args: Parameters<DtaIpcInterface[T]>
  ) {
...
class PurgeTileTreesTool extends Tool {
  ...
  public override async run(modelIds?: string[]): Promise<boolean> {
    ...
    if (undefined !== modelIds && 0 === modelIds.length) modelIds = undefined;
  ...
  public static async startup(
    ...
    IModelApp.applicationLogoCard = () =>
      IModelApp.makeLogoCard({
        iconSrc: "DTA.png",
        iconWidth: 100,
        heading: "Display Test App",
        notice: "For internal testing",
      });
